...
private const val RESTORE_NOTES_DIALOG = 2 //undelete
private const val PERMANENTLY_DELETE_DIALOG = 3 //remove
...
fun TrashScreen(viewModel: MainViewModel) {
    ...
    val notesInThrash: List<NoteModel> by viewModel.notesInTrash //note ท่ี่ลบไปแล้ว
        .observeAsState(listOf())

    val selectedNotes: List<NoteModel> by viewModel.selectedNotes  //note ที่เราจะเลือกว่าจะ undelete ไหม
        .observeAsState(listOf())

    val dialogState = rememberSaveable { mutableStateOf(NO_DIALOG) } //เปิดปิด dialog

    val scaffoldState = rememberScaffoldState() //เปิดตัว app drawer
    ...
    Scaffold(
        ...
        drawerContent = {
            AppDrawer( //content เหมือน หน้าแรก
        ...
        content = {
            Content( // ก็จะเป็น content ที่ลบแล้ว
...
private fun TrashTopAppBar(
    ...
    TopAppBar(
        title = { Text(text = "Tag", color = MaterialTheme.colors.onPrimary) },
        navigationIcon = {
